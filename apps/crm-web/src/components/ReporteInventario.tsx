 ...
 *  (efecto de hojear al cambiar de página) y exportación real a PDF/Excel + impresión — antes
 *  "Generar Reporte"/"Lista de Compras" solo bajaban un CSV sin vista previa ni otros formatos.
 *  Mismo componente en pos-desktop, salvo cómo se guarda el archivo (aquí, File System Access API
 *  del navegador; allá, diálogo nativo de Electron — ver AdminInventario.tsx). */
export function ReporteInventario({ tipo, filas: todasLasFilas, onCerrar }: { tipo: TipoDocumentoInventario; filas: FilaReporte[]; onCerrar: () => void }) {
  ...
  async function exportarPdf() {
    ...
    try {
      ...
      await guardarConDialogoWeb(doc.output("blob"), nombre, [{ description: "Documento PDF", accept: { "application/pdf": [".pdf"] } }]);
    } finally {
      setGenerando("");
    }
  }
  ...
  function enviarPorCorreo() {
    // Limitación real: un enlace mailto: no puede llevar un archivo adjunto (restricción del
    // navegador/sistema operativo, no de esta app) — abre el cliente de correo con el mensaje
    // listo y el PDF descargado aparte para adjuntarlo a mano. Adjuntarlo automáticamente
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 60, padding: 16 }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, maxHeight: "95vh" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, background: "var(--h421-white)", padding: 10, borderRadius: 12 }}>
          ...
          <button onClick={enviarPorCorreo} style={{ background: "transparent", color: "var(--h421-blue)", border: "1px solid var(--h421-blue)", padding: "10px 16px", fontSize: 13 }}>✉️ Enviar por correo</button>
