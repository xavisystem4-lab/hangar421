...
/** Alta, edición y baja de mesas por sucursal — antes no había ninguna pantalla para esto (solo
 *  existía el endpoint POST /mesas, sin usar desde ningún lado), así que una siembra de datos
 *  demo corrida más de una vez sobre la misma sucursal dejaba mesas duplicadas sin forma de
 *  limpiarlas salvo tocando la base a mano. "Eliminar" es baja lógica y el backend la rechaza si
 *  la mesa no está Libre (para no dar de baja por accidente la que sigue en uso ahora mismo). */
            ...
            {mesas.map((m) => {
                ...
                return (
                  <tr key={m.id} style={{ borderBottom: "1px solid var(--h421-gray-200)" }}>
                    ...
                    <td style={{ padding: 8, display: "flex", gap: 6 }}>
                      <button onClick={() => guardarEdicion(m.id)} style={{ background: "var(--h421-green)", color: "#fff", padding: "4px 10px", fontSize: 12 }}>Guardar</button>
                      <button onClick={() => setEditandoId(null)} style={{ background: "var(--h421-gray-200)", padding: "4px 10px", fontSize: 12 }}>Cancelar</button>
                    </td>
                  </tr>
                );
              }
              return (
                <tr key={m.id} style={{ borderBottom: "1px solid var(--h421-gray-200)" }}>
                  ...
                  <td style={{ padding: 8, display: "flex", gap: 6 }}>
                    <button onClick={() => empezarEdicion(m)} style={{ background: "var(--h421-gray-50)", padding: "4px 10px", fontSize: 12 }}>Editar</button>
                    <button onClick={() => eliminar(m)} style={{ background: "var(--h421-red-bg)", color: "var(--h421-red-texto)", padding: "4px 10px", fontSize: 12 }}>Eliminar</button>
                  </td>
                </tr>
              );
            })}
