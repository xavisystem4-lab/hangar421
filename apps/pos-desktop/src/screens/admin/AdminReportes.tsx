...
/** Reportes de ventas con filtros por sucursal y rango de fechas — mismo módulo que
 *  apps/crm-web/reportes, dentro del propio POS para que un admin no tenga que salir a la web. */
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <select value={sucursalId} onChange={(e) => setSucursalId(e.target.value)} style={{ padding: 8, borderRadius: 8, border: "1px solid var(--h421-gray-200)" }}>
            {sucursales.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}
          </select>
          <label style={{ fontSize: 13, color: "var(--h421-gray-600)" }}>Desde</label>
          <input type="date" value={desde} onChange={(e) => setDesde(e.target.value)} style={{ padding: 8, borderRadius: 8, border: "1px solid var(--h421-gray-200)" }} />
          <label style={{ fontSize: 13, color: "var(--h421-gray-600)" }}>Hasta</label>
          <input type="date" value={hasta} onChange={(e) => setHasta(e.target.value)} style={{ padding: 8, borderRadius: 8, border: "1px solid var(--h421-gray-200)" }} />
        </div>
      </div>
