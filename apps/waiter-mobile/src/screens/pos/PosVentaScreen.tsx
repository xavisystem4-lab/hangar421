...
/** Venta de Punto de Venta — mismo catálogo/carrito que `TomaPedidoScreen.tsx` (Comandero), pero
 *  usando `posOrderStore` y con "Cobrar" en vez de "Enviar pedido": igual que el POS Windows
 *  (`apps/pos-desktop/src/screens/POSHome.tsx`), este negocio no tiene un paso separado de
 *  "mandar a cocina" — pagar es lo que crea y envía el pedido en un solo toque (ver
 *  PosCobroScreen.tsx, que llama `enviarACocina()` internamente si todavía no existe un
 *  `pedidoId`). */
export function PosVentaScreen({ nombreCuenta, onCobrar }: { nombreCuenta: string | null; onCobrar: () => void }) {
  const { usuario, sucursalId } = useAuthStore();
  const { items, agregarItem, cambiarCantidad, quitarItem, totales } = usePosOrderStore();
  ...
  const panelResumen = (
    <View style={estilos.resumen}>
      ...
      <TouchableOpacity style={estilos.botonCobrar} onPress={onCobrar} disabled={items.length === 0}>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colores.fondo }}>
      <Text style={estilos.encabezado}>{nombreCuenta ? `Mesa ${nombreCuenta}` : "Pedido de mostrador"}</Text>
...
function crearEstilos(colores: ReturnType<typeof usarColores>, esTablet: boolean, horizontal: boolean) {
  ...
  return StyleSheet.create({
    ...
    productoFoto: { height: 90 * escala, backgroundColor: colores.gray50, alignItems: "center", justifyContent: "center", position: "relative" },
    ...
    botonCobrar: { backgroundColor: colores.green, borderRadius: 12, padding: 16, alignItems: "center", marginTop: 8, minHeight: 56, justifyContent: "center" },
